-- =============================================================================
-- Inbox individual/temporal + Kanban Atendimento + cobrança 8h
-- =============================================================================
--
-- Reestruturação do fluxo operacional:
--   1. Inbox = leads NOVOS de HOJE atribuídos AO consultor logado.
--   2. Atendimento = Kanban (qualificação → proposta → test drive → fechamento).
--   3. Cobrança 8h sem interação humana via SLA Watcher.
--
-- Performance:
--   - Índice composto por (assigned_consultant_id, created_at) WHERE não-final
--   - ultima_interacao_humana pra cobrança SLA 8h

-- ── Coluna nova: ultima_interacao_humana ─────────────────────────────────────
ALTER TABLE leads_manos_crm
    ADD COLUMN IF NOT EXISTS ultima_interacao_humana TIMESTAMPTZ;
...
-- ── Recria leads_unified incluindo ultima_interacao_humana ──────────────────
...
-- ── Índices de performance Inbox ────────────────────────────────────────────
-- Inbox novo carrega O(log n) leads de hoje do consultor logado.
CREATE INDEX IF NOT EXISTS idx_leads_manos_inbox_consultor_data
    ON leads_manos_crm (assigned_consultant_id, created_at DESC)
    WHERE archived_at IS NULL
      AND atendimento_iniciado_em IS NULL
      AND status IN ('received', 'novo', 'triagem');

CREATE INDEX IF NOT EXISTS idx_leads_dist_inbox_consultor_data
    ON leads_distribuicao_crm_26 (assigned_consultant_id, criado_em DESC)
    WHERE archived_at IS NULL
      AND atendimento_iniciado_em IS NULL
      AND status IN ('received', 'novo', 'triagem');

CREATE INDEX IF NOT EXISTS idx_leads_compra_inbox_consultor_data
    ON leads_compra (assigned_consultant_id, criado_em DESC)
    WHERE archived_at IS NULL
      AND atendimento_iniciado_em IS NULL
      AND status IN ('received', 'novo', 'triagem');

-- Índice pro Kanban (Atendimento) — leads em conversa por consultor + status
CREATE INDEX IF NOT EXISTS idx_leads_manos_kanban
    ON leads_manos_crm (assigned_consultant_id, status, ultima_interacao_humana DESC NULLS LAST)
    WHERE atendimento_iniciado_em IS NOT NULL
      AND archived_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_leads_dist_kanban
    ON leads_distribuicao_crm_26 (assigned_consultant_id, status, ultima_interacao_humana DESC NULLS LAST)
    WHERE atendimento_iniciado_em IS NOT NULL
      AND archived_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_leads_compra_kanban
    ON leads_compra (assigned_consultant_id, status, ultima_interacao_humana DESC NULLS LAST)
    WHERE atendimento_iniciado_em IS NOT NULL
      AND archived_at IS NULL;

-- ── Backfill: marca ultima_interacao_humana = updated_at pra leads em atendimento
-- (assim o cron 8h não dispara pra leads antigos sem critério)
UPDATE leads_manos_crm SET ultima_interacao_humana = updated_at
WHERE atendimento_iniciado_em IS NOT NULL AND ultima_interacao_humana IS NULL;

UPDATE leads_distribuicao_crm_26 SET ultima_interacao_humana = atualizado_em
WHERE atendimento_iniciado_em IS NOT NULL AND ultima_interacao_humana IS NULL;

UPDATE leads_compra SET ultima_interacao_humana = updated_at
WHERE atendimento_iniciado_em IS NOT NULL AND ultima_interacao_humana IS NULL;
