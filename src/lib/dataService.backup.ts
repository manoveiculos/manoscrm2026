...
export const dataService = {
    ...
    async getLeads(consultantId?: string, leadId?: string) {
        ...
        if (consultantId) {
            ...
            if (!consultantName) {
                const { data: consultant } = await this.getClient()
        ...
        try {
            ...
            if (consultantId) {
                // Since the VIEW includes both main and crm26, we might need to filter by ID or Name
                // But for now, let's assume assigned_consultant_id is the primary filter
    ...
    async syncAssignedLeadsToMain() {
        ...
        try {
            ...
            const supabaseClient = this.getClient();
            const { data: leadsToSync } = await supabaseClient
            ...
            if (leadsToSync && leadsToSync.length > 0) {
                const { data: consultants } = await supabaseClient.from('consultants_manos_crm').select('id, name');

                for (const lead of leadsToSync) {
                    const consultant = consultants?.find((c: any) => {
                        if (!c.name || !lead.vendedor) return false;
                        return lead.vendedor.toLowerCase().includes(c.name.toLowerCase().split(' ')[0]);
                    });
                    if (consultant) {
                        await this.promoteLeadToMain(lead, 'crm26', consultant.id);
                        await this.markLeadAsSent(lead.id, 'crm26');
                    }
                }
            }

            // 2. Fetch from legacy leads_distribuicao too
            const { data: legacyToSync } = await supabaseClient
            ...
            if (legacyToSync && legacyToSync.length > 0) {
                ...
                for (const lead of legacyToSync) {
                    ...
                    if (consultant) {
                        ...
                        await this.markLeadAsSent(lead.id, 'legacy');
                    }
                }
            }
        } finally {
    ...
    async getLeadsCRM26(consultantName?: string, includeSent: boolean = false, showRedistributed: boolean = false, leadId?: string) {
        ...
        if (!allConsultants) {
            const { data: consultantsData } = await supabaseClient.from('consultants_manos_crm').select('id, name');
    ...
    async getDistributedLeads(consultantId?: string) {
        ...
        let queryMain = supabase
            .from('leads_manos_crm')
            .select('*')
            .or('status.in.(closed,lost,post_sale,lost_redistributed,lixo,duplicado,sem contato,perda total),status.ilike.%perca%,status.ilike.%perdido%');
    ...
    async updateDistributedLeadAI(id: any, aiData: any, sourceTable: string = 'leads_distribuicao') {
        ...
        if (error) {
            ...
            await supabaseClient.from(sourceTable).update({ [resumoColumn]: finalResumo }).eq('id', id);
        }
    },
    ...
    async distributeOldLeads(leadsToDistribute: { id: string | number, currentConsultant?: string, currentConsultantId?: string }[], vendedores: string[]) {
        ...
        const { data: allConsultants } = await supabaseClient.from('consultants_manos_crm').select('id, name');
        ...
        const updates = leadsToDistribute.map((leadInfo, index) => {
            ...
            if (table === 'leads_manos_crm') {
                ...
                return supabaseClient.from(table).update(updatePayload).eq('id', realId);
            }
            ...
            if (firstError.code === '42703' || (firstError.message && firstError.message.toLowerCase().includes('atualizado_em'))) {
                ...
                const retryUpdates = leadsToDistribute.map((leadInfo, index) => {
                    ...
                    if (table === 'leads_manos_crm') {
                        ...
                        return supabaseClient.from(table).update(payload).eq('id', realId);
                    }
    ...
    async updateLeadStatus(leadId: string, status: LeadStatus, oldStatus?: LeadStatus, notes?: string, motivo_perda?: string, resumo_fechamento?: string) {
        ...
        if (isRedistributionTrigger) {
            ...
            try {
                ...
                const { data: currentLead } = await supabaseClient.from(table).select('*, consultants_manos_crm(name)').eq('id', realId).single();
                ...
                const { error: redError } = await supabaseClient
                    .from(table)
        ...
        if (error) {
            ...
            if (table === 'leads_distribuicao_crm_26') {
                const { data } = await supabaseClient.from(table).select('resumo').eq('id', realId).single();
                ...
                const { error: fallbackError } = await supabaseClient.from(table).update({ resumo, status: targetStatus }).eq('id', realId);
                if (fallbackError) throw fallbackError;
            } else {
                // Secondary fallback for other tables, try updating just the status
                const { error: fallbackError } = await supabaseClient.from(table).update({ status: targetStatus }).eq('id', realId);
                if (fallbackError) throw fallbackError;
            }
        }
        ...
        if (table === 'leads_manos_crm' || table === 'leads_distribuicao_crm_26') {
            ...
            try {
                const { data: leadData } = await supabaseClient.from(table).select('*').eq('id', realId).single();
    ...
    async getFinancialMetrics(period: string = 'this_month', customRange?: { start: string, end: string }) {
        ...
        let leadsQuery = supabaseClient.from('leads').select('*', { count: 'exact', head: true }).gte('created_at', startDate);
        let salesQuery = supabaseClient.from('leads').select('valor_investimento').in('status', soldStatuses).gte('updated_at', startDate);
        let formalSalesQuery = supabaseClient.from('sales').select('amount').gte('sale_date', startDate);

        if (endDate) {
            leadsQuery = leadsQuery.lte('created_at', endDate);
            salesQuery = salesQuery.lte('updated_at', endDate);
            formalSalesQuery = formalSalesQuery.lte('sale_date', endDate);
        }

        const [campaignsRes, leadsCountRes, salesRes, formalSalesRes] = await Promise.all([
            supabaseClient.from('campaigns').select('total_spend'),
            leadsQuery,
            salesQuery,
            formalSalesQuery
        ]);
        ...
        const formalSalesData = formalSalesRes.data || [];
        
        // Map formal sales to the same format as lead sales
        const mappedFormalSales = formalSalesData.map((s: any) => ({ valor_investimento: s.amount }));
        const combinedSales = [...salesData, ...mappedFormalSales];
    ...
    async getRecentSales(limit: number = 5) {
        try {
            ...
            return (data || []).map(s => ({
                id: s.id,
                lead: { name: s.vehicle_name || 'Venda Direta' }, // Ajuste conforme necessário
    ...
    async getConsultantMetrics(consultantId: string) {
        ...
        if (consultant?.name) {
            ...
            const { data: leads26 } = await supabaseClient
                .from('leads_distribuicao_crm_26')
        ...
        const { data: salesMonth } = await supabaseClient
        ...
        const { data: salesAll } = await supabaseClient
            .from('sales_manos_crm')
            .select('sale_value, profit_margin')
            .eq('consultant_id', consultantId);
        ...
        if (consultant?.name) {
            const firstName = consultant.name.trim().split(' ')[0];
            const { data: crm26Scheduled } = await supabaseClient
                .from('leads_distribuicao_crm_26')
                .select('*')
                .ilike('vendedor', `%${firstName}%`)
                .neq('resumo', null)
    ...
    async getConsultantPerformance() {
        ...
        // Fetch counts EXCLUSIVAMENTE from CRM 26 for each consultant using the unified 'leads' View
        const performance = await Promise.all((consultants || []).map(async (c: any) => {
            const firstName = c.name ? c.name.trim().split(' ')[0] : '';
            const { count: count26 } = await supabaseClient
                .from('leads')
                .select('*', { count: 'exact', head: true })
                .eq('source_type', 'crm26')
                .ilike('name', `%${firstName}%`); // Note: In the View, 'nome' is mapped to 'name'
    ...
    async getLeadsCountByDateForCampaigns(datePreset: string) {
        ...
        const { data: data26 } = await supabaseClient
            .from('leads_distribuicao_crm_26')
        ...
        const { data: fbLeads } = await supabaseClient
            .from('leads_distribuicao_crm_26')
        ...
        // Get all campaigns
        const { data: allCampaigns } = await supabaseClient.from('campaigns_manos_crm').select('id, name, platform');
    ...
    async reassignDistributedLead(leadId: string | number, newVendedorName: string) {
        ...
        const { error } = await supabaseClient.from(table).update(updatePayload).eq('id', realId);
        ...
        if (error) {
            ...
            if (error.code === '42703' || (error.message && error.message.toLowerCase().includes('atualizado_em'))) {
                ...
                const { error: retryError } = await supabaseClient.from(table).update(updatePayload).eq('id', realId);
                if (retryError) throw retryError;
            } else {
                throw error;
            }
    ...
    async restaurarLead(leadId: any, sourceTable: string) {
        const now = new Date().toISOString();
        const payload = {
            status: 'received',
            updated_at: now,
            created_at: now // Bumps to top of list as "New"
        };
        ...
        try {
            ...
            const { error } = await supabaseClient
                .from(sourceTable)
                .update(sourceTable === 'leads_manos_crm' ? payload : { 
                    status: 'received', 
                    atualizado_em: now, 
                    criado_em: now 
                })
