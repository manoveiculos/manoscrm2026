                    ...
                    <table className="w-full text-left border-collapse table-fixed min-w-[860px]">
                        ...
                        <tbody>
                            {leads.map((lead, idx) => {
                                ...
                                const rawAssignedId = lead.assigned_consultant_id?.toString() || '';
                                const cleanConsultantId = rawAssignedId.replace(/^(main_|crm26_|dist_|lead_|crm25_|master_|compra_)/i, '').toLowerCase().trim();
                                
                                let assignedConsultant = consultants?.find(c => c.id.toLowerCase().trim() === cleanConsultantId);
                                
                                // Fallback robusto caso o ID seja o próprio nome legado
                                if (!assignedConsultant && rawAssignedId && !/^[0-9a-f]{8}-/i.test(cleanConsultantId)) {
                                    assignedConsultant = consultants?.find(c => c.name.toLowerCase().includes(rawAssignedId.toLowerCase().trim()));
                                }

                                const consultantDisplayName = assignedConsultant?.name || lead.consultant_name || lead.vendedor || lead.primeiro_vendedor || '—';
