...
// CORRECTED SQL based on actual schema:
// leads_master:   city (not region), next_step (not proxima_acao), primeiro_vendedor
// leads_manos_crm: region, proxima_acao, resumo_consultor, vendedor_anterior
// leads_distribuicao_crm_26: telefone, nome, cidade, criado_em, atualizado_em, resumo_consultor
const FIX_SQL = `
...
WITH all_sources AS (
  ...
  -- Source 1: leads_master (priority 1)
  SELECT
  ...
  SELECT
    ...
    COALESCE(d.atualizado_em, d.criado_em, NOW())  AS updated_at,
...
async function main() {
  console.log('=== Applying CORRECTED leads VIEW ===\n');
  ...
  if (result.status === 200 || result.status === 201) {
    ...
    const { data, error, count } = await sb.from('leads').select('*', { count: 'exact' }).limit(1);
    if (error) {
      console.error('❌ Erro ao verificar:', error.message);
    } else {
      if (data && data.length > 0) {
        console.log('✅ Colunas da nova VIEW:', Object.keys(data[0]).join(', '));
      }
      console.log(`✅ Total de leads na VIEW: ${count}`);
    }
  } else {
    console.error('❌ Falha:', result.body.slice(0, 500));
  }
}
